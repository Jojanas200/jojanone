import { NextResponse } from "next/server";
import { getClaims } from "@/server/auth/session";
import { getActiveWorkspaceId } from "@/server/services/workspaces";
import {
  createTenderOpportunity,
  listTenderOpportunities,
} from "@/server/services/tender";
import { createTenderOpportunitySchema } from "@/shared/schemas/tender";

export async function GET() {
  const claims = await getClaims();
  if (!claims)
    return NextResponse.json({ error: "unauthorized" }, { status: 401 });
  const rows = await listTenderOpportunities(claims);
  return NextResponse.json({ opportunities: rows });
}

export async function POST(req: Request) {
  const claims = await getClaims();
  if (!claims)
    return NextResponse.json({ error: "unauthorized" }, { status: 401 });

  const ws = await getActiveWorkspaceId(claims);
  if (!ws)
    return NextResponse.json({ error: "no active workspace" }, { status: 400 });

  const body = await req.json().catch(() => null);
  const parsed = createTenderOpportunitySchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid opportunity", issues: parsed.error.issues },
      { status: 400 },
    );
  }

  const created = await createTenderOpportunity(claims, ws, parsed.data);
  return NextResponse.json({ opportunity: created }, { status: 201 });
}
