...
import { updateEntitySchema } from "@/shared/schemas/business-entities";
...
export async function PATCH(req: Request, { params }: Ctx) {
  ...
  const updated = await updateEntity(claims, id, parsed.data);
  if (!updated)
...
export async function DELETE(_req: Request, { params }: Ctx) {
  ...
  if (!claims)
    return NextResponse.json({ error: "unauthorized" }, { status: 401 });
  const ok = await deleteEntity(claims, id);
